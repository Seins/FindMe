...
app.ui.doFrameLayout = function(svrName){
	...
	_ui._appHeader = $(".mgrHeader");
	_ui._appLogo = $(".mgrLogo");
	_ui._appCmd = $(".appCmd");
...
app.ui._initFrame = function(){
	...
	_mainTab.addTab("_portal", "管理首页", "80px");
	_mainTab.setHrefMode("iframes");
	_mainTab.setContentHref("_portal", app.serviceName+"/manager/portal.htm");
...
app.ui.toggleHeader = function(){
	if(!app.ui._headerCollapsed){
		...
		app.ui._appLogo.attr("class", "mgrMiniLogo");
		...
		app.ui._appLogo.attr("class", "mgrLogo");
...
app.changeUserPwd = function(){
	...
	win.attachURL(app.serviceName + "/manager/modifyPwd.htm");
	win.setText("修改密码");
}

app.logout = function(){
	if(confirm("你确认要注销吗？")){
		window.location = app.serviceName + "/manager/logout.htm";
	}
}
