...
app.ui.doFrameLayout = function(svrName, subSysTag, subSysPortal){
	if(app.isInit)
		return;
	app.serviceName = svrName;
	app.subSysTag = subSysTag;
	app.subSysPortal = subSysPortal;
	var _ui = app.ui;
	//== Main Layout ==
	...
	_ui.menuBar = _ui.layoutMenu.attachAccordion();
...
app.ui._initFrame = function(){
	...
	_mainTab.addTab("_portal", "子系统首页", "80px");
	_mainTab.setHrefMode("iframes");
	_mainTab.setContentHref("_portal", app.subSysPortal?((app.subSysPortal.startsWith("/")?app.serviceName:'')+app.subSysPortal):(app.serviceName+"/subsys/portal.htm?subSysTag="+app.subSysTag));
	...
	menu.setIconsPath(app.serviceName + "/resources/images/");
...
app.changeUserPwd = function(){
	...
	win.attachURL(app.serviceName + "/subsys/modifySubPwd.htm?subSysTag=" + app.subSysTag);
	win.setText("修改密码");
}

app.logout = function(){
	if(confirm("你确认要注销吗？")){
		window.location = app.serviceName + "/subsys/logout.htm?subSysTag=" + app.subSysTag;
	}
}
