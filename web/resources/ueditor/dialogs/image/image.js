...
(function () {
    ...
    function addManagerListener(){
      g("imgRefreshBtn").onclick = function () {
          refreshImg();
      };
      g("imgDeleteBtn").onclick = function () {
          deleteImg();
      };
    }
    ...
    function deleteImg(){
      var imgs = $G("imageList").getElementsByTagName("img"), urls = [] ;
      for (var i = 0, ci; ci = imgs[i++];) {
          if (ci.getAttribute("selected")) {
      ...
      if(urls.length==0)
        return;
      if(!confirm("删除图片将使引用图片的页面展现异常，请确认是否继续删除？"))
        return;
      ajax.request(editor.options.imageManagerDeleteUrl, {
          timeout:100000,
          action:"post",
          data:{"imgs": urls.join("ue_separate_ue")},
          onsuccess:function (xhr) {
              //去除空格
              var tmp = utils.trim(xhr.responseText);
              if(tmp && tmp.indexof("error:")==0){
                alert(tmp.substr(6));
              }else{
                refreshImg();
              }
